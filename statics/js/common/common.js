...
function AjaxRequest(url,data,callBack,sync,dataCallBack,_this){
	layui.use(['layer','fsCommon','fsConfig'],function(){
		...
		fsCommon.invoke(url,data,function(data){
			if(data[statusName] == successNo)
			{
				if (window[callBack]){
					window[callBack].call(_this,data);
				}else{
					callBack(data);
				}
			}
